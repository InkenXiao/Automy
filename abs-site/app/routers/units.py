...
async def list_units(db: AsyncSession = Depends(get_db)):
    ...
    stmt = (
        select(Unit)
        .where(Unit.is_delete.is_(False))
        .options(
            selectinload(Unit.words),
...
async def get_unit(unit_id: int, db: AsyncSession = Depends(get_db)):
    ...
    stmt = (
        ...
        .where(Unit.id == unit_id, Unit.is_delete.is_(False))
    )
    result = await db.execute(stmt)
...
async def create_unit(unit_in: UnitCreate, db: AsyncSession = Depends(get_db)):
    ...
    stmt = (
        ...
        .where(Unit.id == unit.id, Unit.is_delete.is_(False))
    )
    result = await db.execute(stmt)
...
async def update_unit(
    ...
    result = await db.execute(stmt)
    unit = result.scalar_one_or_none()
    if unit is None or unit.is_delete:
        raise HTTPException(status_code=404, detail="Unit not found")
