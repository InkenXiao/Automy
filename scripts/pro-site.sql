...
COMMENT ON TABLE  projects IS '项目元信息 (一个项目对应一张进度计划执行图, 多项目隔离)';
...
COMMENT ON COLUMN projects.is_active IS '是否为当前激活项目 (业务接口默认用激活项目)';
...
COMMENT ON TABLE  modules IS '项目模块字典 (底座/数据/智能体/应用/需求/协调, 按项目隔离)';
COMMENT ON COLUMN modules.id IS '主键ID, 自增';
COMMENT ON COLUMN modules.project_id IS '★所属项目ID, 关联 projects.id';
...
COMMENT ON COLUMN modules.color IS '模块主题色 (HEX)';
COMMENT ON COLUMN modules.color_bg IS '模块背景色 (HEX)';
...
COMMENT ON TABLE  phases IS '项目阶段字典 (第一阶段/第二阶段/第三阶段, 按项目隔离)';
COMMENT ON COLUMN phases.id IS '主键ID, 自增';
COMMENT ON COLUMN phases.project_id IS '★所属项目ID, 关联 projects.id';
COMMENT ON COLUMN phases.name IS '阶段名称';
...
CREATE TABLE IF NOT EXISTS progress_tasks (
    ...
    updated_at    TIMESTAMP    DEFAULT NOW(),
    is_delete     BOOLEAN      DEFAULT FALSE,
    CONSTRAINT fk_progress_tasks_project FOREIGN KEY (project_id) REFERENCES projects(id) ON DELETE CASCADE,
    CONSTRAINT fk_progress_tasks_phase   FOREIGN KEY (phase_id)   REFERENCES phases(id)
);
CREATE INDEX IF NOT EXISTS idx_progress_tasks_project_id ON progress_tasks(project_id);
CREATE INDEX IF NOT EXISTS idx_progress_tasks_phase_id   ON progress_tasks(phase_id);
COMMENT ON TABLE  progress_tasks IS '项目进度计划任务 (执行图任务节点, 按项目隔离)';
COMMENT ON COLUMN progress_tasks.id IS '主键ID, 自增';
COMMENT ON COLUMN progress_tasks.project_id IS '★所属项目ID, 关联 projects.id';
...
COMMENT ON COLUMN progress_tasks.status IS '状态: ongoing(进行中)/planned(计划中)/milestone(里程碑)/done(已完成)';
...
COMMENT ON COLUMN meetings.project_id IS '★所属项目ID, 关联 projects.id';
...
COMMENT ON COLUMN meetings.sort_order IS '排序序号';
COMMENT ON COLUMN meetings.is_delete IS '是否已软删除';
...
CREATE TABLE IF NOT EXISTS meeting_items (
    ...
    updated_at   TIMESTAMP    DEFAULT NOW(),
    is_delete    BOOLEAN      DEFAULT FALSE,
    CONSTRAINT fk_meeting_items_meeting FOREIGN KEY (meeting_id) REFERENCES meetings(id) ON DELETE CASCADE
);
CREATE INDEX IF NOT EXISTS idx_meeting_items_meeting_id ON meeting_items(meeting_id);
...
COMMENT ON COLUMN meeting_items.duration IS '议程时长';
...
CREATE TABLE IF NOT EXISTS weekly_reports (
    ...
    updated_at        TIMESTAMP    DEFAULT NOW(),
    is_delete         BOOLEAN      DEFAULT FALSE,
    CONSTRAINT fk_weekly_reports_project FOREIGN KEY (project_id) REFERENCES projects(id) ON DELETE CASCADE
);
...
COMMENT ON COLUMN weekly_reports.project_id IS '★所属项目ID, 关联 projects.id (复制上周周报时继承源周报)';
...
CREATE TABLE IF NOT EXISTS weekly_kpis (
    ...
    is_delete    BOOLEAN     DEFAULT FALSE,
    CONSTRAINT fk_weekly_kpis_report FOREIGN KEY (report_id) REFERENCES weekly_reports(id) ON DELETE CASCADE,
    CONSTRAINT fk_weekly_kpis_module FOREIGN KEY (module_id) REFERENCES modules(id),
...
CREATE TABLE IF NOT EXISTS weekly_progress_items (
    id          SERIAL PRIMARY KEY,
    report_id   INTEGER      NOT NULL,
    module_id   INTEGER,
    ...
    sort_order  INTEGER      DEFAULT 0,
    is_delete   BOOLEAN      DEFAULT FALSE,
    CONSTRAINT fk_weekly_progress_items_report FOREIGN KEY (report_id) REFERENCES weekly_reports(id) ON DELETE CASCADE,
    CONSTRAINT fk_weekly_progress_items_module FOREIGN KEY (module_id) REFERENCES modules(id)
);
...
CREATE TABLE IF NOT EXISTS weekly_plan_tasks (
    ...
    updated_at        TIMESTAMP    DEFAULT NOW(),
    is_delete         BOOLEAN      DEFAULT FALSE,
    CONSTRAINT fk_weekly_plan_tasks_report        FOREIGN KEY (report_id)        REFERENCES weekly_reports(id)  ON DELETE CASCADE,
    CONSTRAINT fk_weekly_plan_tasks_module        FOREIGN KEY (module_id)        REFERENCES modules(id),
    CONSTRAINT fk_weekly_plan_tasks_progress_task FOREIGN KEY (progress_task_id) REFERENCES progress_tasks(id)
);
...
CREATE TABLE IF NOT EXISTS weekly_risks (
    id             SERIAL PRIMARY KEY,
    report_id      INTEGER      NOT NULL,
    ...
    sort_order     INTEGER      DEFAULT 0,
    is_delete      BOOLEAN      DEFAULT FALSE,
    CONSTRAINT fk_weekly_risks_report FOREIGN KEY (report_id) REFERENCES weekly_reports(id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS weekly_work_tasks (
    id              SERIAL PRIMARY KEY,
    project_id      INTEGER      NOT NULL,
    week_start      DATE,
    week_end        DATE,
    ...
    updated_at      TIMESTAMP    DEFAULT NOW(),
    is_delete       BOOLEAN      DEFAULT FALSE,
    CONSTRAINT fk_weekly_work_tasks_project   FOREIGN KEY (project_id)    REFERENCES projects(id)          ON DELETE CASCADE,
    CONSTRAINT fk_weekly_work_tasks_plan_task FOREIGN KEY (plan_task_id)  REFERENCES weekly_plan_tasks(id),
    CONSTRAINT fk_weekly_work_tasks_module    FOREIGN KEY (module_id)     REFERENCES modules(id)
);
...
COMMENT ON COLUMN weekly_work_tasks.project_id IS '★所属项目ID, 关联 projects.id (从周报批量生成时继承周报)';
...
-- ---------- 项目 ----------
INSERT INTO projects (id, name, title, based_doc, start_date, end_date, is_active, sort_order)
VALUES
...
INSERT INTO modules (id, project_id, idx, tag, title, owner, color, color_bg, sort_order)
VALUES
...
INSERT INTO phases (id, project_id, name, subtitle, description, start_date, end_date)
VALUES
...
-- ============================================================
-- 脚本执行完成
-- 表清单: projects / modules / phases / progress_tasks
--         / meetings / meeting_items
--         / weekly_reports / weekly_kpis / weekly_progress_items
--         / weekly_plan_tasks / weekly_risks / weekly_work_tasks
-- ============================================================
