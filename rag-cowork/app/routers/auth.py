"""身份确认路由 · 姓名登录 (可选密码), 校验 sys_users 共享用户表

密码逻辑与 pro-cowork 一致 (pbkdf2 加盐, salt$hash 存储):
- 未设密码: 姓名直登
- 已设密码: 需携带 password; 未带返回 need_password=true
"""
import hashlib
import secrets
...
async def list_users(db: AsyncSession = Depends(get_db)) -> dict:
    """用户列表 (知识库授权下拉用)"""
    result = await db.execute(
        select(SysUser).where(SysUser.is_delete.is_(False), SysUser.is_active.is_(True)).order_by(SysUser.name)
    )
    return {
        "users": [
